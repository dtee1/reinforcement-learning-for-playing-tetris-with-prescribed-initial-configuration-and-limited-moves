...
def get_tetromino(piece: int, rotations: int):
...
class RandomPieceGenerator:
    ...
    def generate_pieces(self, level: int = None) -> None:
            self.pieces = list(range(7))


    def _regenerate(method: Callable):
        def wrapper(self, *args, **kwargs):
            regenerated = False

            if not self.pieces:
                self.generate_pieces(1)
                regenerated = True
    ...
    def get_random_sequence(self, length: int):
        sequence = []
...
class Tetris_Generator:
    def __init__(self, L, M, random_pieces=False, max_revert=10, max_checkpoint=40):
    ...
    def __get_initial_config(self) -> None:
        ...
        initial_empty = random.randint(14,16)
        ...
        while (len(self.pieces) < self.M) and np.count_nonzero(self.board[-1]) >= len(self.board[-1]) / 2:
            ...
            if self.carve(random_piece, rotations, location, len(self.random_piece_generator) == 7):
                ...
                if checkpoint_manager.add_attempt():
                    ...
                    self.random_piece_generator.generate_pieces(1)
    ...
    def calculate_carve(self, drop: int, location: int, tetromino: np.array,
                        reverse_tetromino_topography, allow_partial: bool):
        tetromino_height, tetromino_width = tetromino.shape
    ...
    def get_state(self):
        return self.board, self.pieces[0], self.pieces[
            1], self.L - self.lines_cleared, self.M - self.moves_used, self.state
